...
extension Notification.Name {
    static let didRunDigest = Notification.Name("didRunDigest")
}
...
struct PreferencesView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .task { await load(); await loadStatus() }
    ...
    private func loadStatus() async {
        do {
            let statusResponse = try await APIClient.shared.getStatus()
            guard let runAt = statusResponse.lastRunAt, !runAt.isEmpty else { return }
            let state = statusResponse.lastRunStatus ?? "unknown"
            status = "Последний прогон: \(runAt) [\(state)]"
        } catch {
            // status endpoint is optional for UI flow
        }
    }
    ...
    private func runNow() async {
        do {
            ...
            let runMessage = try await APIClient.shared.runDigest()

            var refreshWarning: String? = nil
            do {
                _ = try await APIClient.shared.getSummariesToday()
                NotificationCenter.default.post(name: .didRunDigest, object: nil)
            } catch {
                refreshWarning = " Данные " +
                    "вкладки «Сегодня» обнови кнопкой «Обновить»."
            }
            ...
            if let runMessage {
                status = "\(runMessage) ✅" + (refreshWarning ?? "")
            } else {
                status = "Готово ✅ Дайджест собран." + (refreshWarning ?? "")
            }
        } catch {
            isLoading = false
